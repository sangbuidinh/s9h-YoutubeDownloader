...
- Fast mode includes yt-dlp `--ignore-errors` to match the reference BAT. If yt-dlp still returns a non-zero status but leaves a non-empty staged MP4, the application attempts the BAT-compatible FFmpeg transcode and accepts the video only if FFmpeg succeeds, Premiere-safe validation passes, and atomic promotion succeeds. If no usable staged MP4 exists, the original yt-dlp error is propagated.
