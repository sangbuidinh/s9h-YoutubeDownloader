...
$ExeHash = Get-FileHash -LiteralPath $StandaloneExe -Algorithm SHA256
$ZipHash = Get-FileHash -LiteralPath $ZipPath -Algorithm SHA256
$ZipExeHash = Get-FileHash -LiteralPath (Join-Path $VerifyRoot "Youtube.Downloaderbs.exe") -Algorithm SHA256
if ($ExeHash.Hash -ne $ZipExeHash.Hash) { throw "Standalone EXE differs from the EXE inside the ZIP" }
...
Copy-Item -LiteralPath (Join-Path $RepoRoot "docs\release_notes_v1.3.0-rc.1.md") -Destination $NotesPath
@"
...
"@ | Add-Content -LiteralPath $NotesPath -Encoding utf8
...
Write-Host "Prerelease assets are ready in release/assets"
