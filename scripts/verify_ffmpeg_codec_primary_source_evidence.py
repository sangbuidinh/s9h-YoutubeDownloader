...
def _verify_inventory(root: Path, primary: dict[str, Any], inventory: dict[str, Any]) -> None:
    ...
    _require(normalized == baseline, "inventory changed outside selected evidence and blockers")
...
def _verify_feasibility(root: Path, paths: dict[str, Path], document: dict[str, Any], raw: bytes) -> None:
    _require(hashlib.sha256(raw).hexdigest() == FEASIBILITY_HASH, "feasibility hash changed")
...
def _verify_artifacts(
    ...
    changed = _phase_changed_paths(root)
    _require(changed == PHASE_PATHS, "phase changed-file scope is not exact 9/9")
    if introduced_paths is None:
        introduced_paths = sorted(changed)
...
def _phase_changed_paths(root: Path) -> set[str]:
